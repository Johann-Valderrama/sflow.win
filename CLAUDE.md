...
| Backend local falla: "cublas64_12.dll not found"       | ctranslate2 requiere CUDA Toolkit 12.x (`cublas64_12.dll`). Si tienes CUDA 13.x sin 12.x, el blindaje `_ensure_cuda_on_path()` busca bajo `v12.*` y antepone su carpeta al PATH. Si sigue fallando, descarga CUDA 12.9 desde nvidia.com; el 13.x y 12.x coexisten. Como último recurso, `LOCAL_DEVICE=cpu` fuerza CPU. Consulta `docs/benchmarks/local-backend-gpu-2026-08-01.md` para ver si CUDA vale la pena en tu máquina. |
| Modelo `medium` muy lento / no termina a tiempo en CPU | En CPU, `medium` corre **más lento que tiempo real** para clips cortos (x0.7-x1.1): inutilizable para dictado. CUDA int8 lo lleva a x4.5-x7.6 (viable). Si no tienes GPU, usa `small` (default). Con CUDA 12.x sí se puede subir a `LOCAL_WHISPER_MODEL=medium` en `.env` (reiniciando), pero **el default sigue en `small` a propósito**: cambiarlo por defecto está pendiente de decisión, porque `medium` solo es usable con GPU. Sin CUDA, `medium` en `local` está descartado; usa `groq` en la nube o Groq como respaldo (`GROQ_FALLBACK=true`). |
