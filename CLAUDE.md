...
| Modelo `medium` muy lento / no termina a tiempo en CPU | En CPU, `medium` corre **más lento que tiempo real** para clips cortos (x0.7-x1.1): inutilizable para dictado. CUDA int8 lo lleva a x4.5-x7.6 (viable). Si no tienes GPU, usa `small` (default). Con CUDA 12.x, cambia `LOCAL_WHISPER_MODEL=medium` en `.env` y reinicia. Sin CUDA, `medium` en `local` está descartado; usa `groq` en la nube o Groq como fallback (`GROQ_FALLBACK=true`). |
