...
- **DECIDIDO (Johann, 2026-08-01): el default de `LOCAL_WHISPER_MODEL` SE QUEDA en `small`.** No se re-abre sin que él lo pida. Razón de la opción elegida: `small` con GPU ya va muy rápido, y el default tiene que seguir siendo seguro en una máquina SIN tarjeta, donde `medium` es inutilizable. `medium` queda disponible a un cambio de línea en `.env` para quien lo quiera; ya está descargado en `models/`. Lo medido que sostuvo la decisión (7a, 2026-08-01, ver `docs/benchmarks/local-backend-gpu-2026-08-01.md`): en CPU `medium` corre x0.7-x1.1 tiempo real (más lento que el propio audio, no se puede dictar), en CUDA x4.5-x7.6.
