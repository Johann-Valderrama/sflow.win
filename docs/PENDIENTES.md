...
- **🙋 DECISIÓN PENDIENTE DE JOHANN: ¿el default de `LOCAL_WHISPER_MODEL` pasa de `small` a `medium`?** La Ola 7 solo MIDIÓ que ahora se puede; nadie ha decidido que se haga, y por eso el default sigue en `small`. Lo medido (7a, 2026-08-01, ver `docs/benchmarks/local-backend-gpu-2026-08-01.md`): en CPU, `medium` corre más lento que tiempo real en clips cortos (x0.7-x1.1), o sea inutilizable para dictado; en CUDA corre a x4.5-x7.6, tan usable como `small` en CPU hoy. El modelo ya está descargado en `models/`. Lo que hay que pesar es que `medium` es más preciso pero solo sirve en máquinas con GPU: cambiarlo por defecto degrada a quien no la tenga.
  - **Límite de fidelidad, dicho con precisión** (no convertir en "son igual de buenos"): entre CPU y CUDA el texto es idéntico o casi por debajo de 30 s, y difiere ~1,8% de caracteres en clips de 60 s (ratio 0,9816). Los dos dispositivos son deterministas consigo mismos (ratio 1,0 en dos cargas independientes) y quedan a la MISMA distancia de un modelo de referencia más fuerte (`large-v3-turbo` como proxy, no como verdad humana). O sea: **no hay evidencia de que CUDA transcriba peor, tampoco de que transcriba mejor.** La CAUSA de esa divergencia NO se midió y queda declarada como límite, no explicada: cualquier explicación (deriva numérica, segmentación de VAD) sería una hipótesis sin comprobar.
