...
- 2026-07-31 **Contrato del pipeline de texto, tres ejes y no solo el orden** (commits 40ca392 +
  ...
  smart commands va antes que snippets (el texto guardado de un snippet ya viene puntuado y no debe
  re-escanearse); las pasadas del dictado se cablean en `main.py` y JAMAS en `core/transcriber.py`;
  y `raw_text` significa "lo que devolvio el backend, antes de toda pasada local", con el tradeoff
  aceptado de que el Undo tambien revierta la puntuacion pedida por voz. La regla DURABLE, que
  sobrevive a la tabla de alcance: una pasada que reinterpreta lo que el hablante quiso ESCRIBIR
  solo corre donde el hablante es el usuario. Detalle: `CLAUDE.md` seccion 19 (fuente de verdad);
  registro de por que: `docs/PLAN-DICTADO-2026-07-31.md`, Ola 0. : gatillo Ola 0 del PLAN-DICTADO
  : @opus-5 (dir + 0a) + @sonnet-5 (0b)
